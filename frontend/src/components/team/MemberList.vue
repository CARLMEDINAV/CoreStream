          ...
          >
            ...
            <td class="px-4 py-4">
              <div class="flex items-center justify-center gap-2">
                ...
                <button
                  v-if="isAdmin && member.role === 'TEAM_LEADER'"
    ...
    <div v-if="memberToDelete" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-xl p-8 max-w-md w-full mx-4">
        ...
        <p class="text-gray-600 mb-6 text-sm">
          Estás a punto de eliminar a este miembro del equipo. Por favor, elige cómo deseas proceder con sus datos:
        </p>
        ...
        <div class="flex flex-col gap-3">
          ...
          <button 
            ...
            class="w-full px-4 py-3 bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-lg hover:bg-yellow-100 transition-colors text-left"
          >
            <span class="font-bold block">Desactivar (Soft Delete)</span>
            <span class="text-xs text-yellow-600">Recomendado. Conserva su historial de tickets para las métricas.</span>
          </button>

          <button 
            @click="confirmDelete(true)" 
            class="w-full px-4 py-3 bg-red-50 border border-red-200 text-red-800 rounded-lg hover:bg-red-100 transition-colors text-left"
          >
            <span class="font-bold block">Eliminación Total (Hard Delete)</span>
            <span class="text-xs text-red-600">Peligroso. Destruye su registro y tickets asociados permanentemente.</span>
          </button>

          <button 
            @click="memberToDelete = null" 
            class="w-full mt-2 px-4 py-2 border-2 border-gray-200 rounded-lg font-semibold text-gray-700 hover:bg-gray-50 transition-colors text-center"
          >
            Cancelar y volver
          </button>
        </div>
...
const emit = defineEmits<{
   ...
   */
  delete: [userId: string, hardDelete: boolean]
}>()
...
const filteredMembers = computed<TeamMember[]>(() => {
  if (!searchQuery.value.trim()) {
    return members.value as TeamMember[]
  }
...
function getAvatarColor(role: string): string {
  const colorMap: Record<string, string> = {
    ADMIN: '#DC2626', // Rojo
    TEAM_LEADER: '#D97706', // Oro/Amarillo
...
/**
 * Funciones que manejan la acción de eliminar un miembro
 * Pregunta al usuario qué tipo de eliminación desea hacer
 */
// Variable para saber si mostramos el modal y a quién vamos a borrar
const memberToDelete: Ref<string | null> = ref(null)

// Función que abre el modal y guarda el ID del usuario
function handleDelete(userId: string): void {
  memberToDelete.value = userId
}

// Esta función se llama al hacer clic en uno de los botones del modal
function confirmDelete(hardDelete: boolean): void {
  if (memberToDelete.value) {
    // Le pasamos la pelota al Jefe (TeamView.vue)
    emit('delete', memberToDelete.value, hardDelete)
    // Cerramos el modal
...
async function handleAddMember(): Promise<void> {
  try {
    ...
    if (!newMember.value.fullName || !newMember.value.email) {
    ...
    await teamStore.addMember({
      ...
      role: UserRole.DEVELOPER // Los nuevos miembros siempre comienzan como desarrolladores
    })
