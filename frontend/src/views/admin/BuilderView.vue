      ...
      <div class="w-full max-w-xl rounded-[2rem] border border-[var(--border-subtle)] bg-[var(--bg-app)] p-6 shadow-2xl shadow-black/40">
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <h3 class="mt-1 text-2xl font-semibold text-[var(--text-primary)]">{{ editingAppId ? t('builderView.editApplication') : t('builderView.newApplicationTitle') }}</h3>
          </div>
          <button type="button" class="rounded-full border border-[var(--border-subtle)] bg-[var(--bg-panel)] px-3 py-1 text-sm text-[var(--text-secondary)]" @click="closeAppModal">
            {{ t('builderView.close') }}
          </button>
        </div>

        <form class="mt-6 space-y-4" @submit.prevent="saveApp">
          <div>
            <label class="mb-1 block text-sm text-[var(--text-secondary)]">{{ t('builderView.formName') }}</label>
            <input data-cy="app-name-input" v-model="appForm.name" type="text" class="w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-panel)] px-4 py-3 text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:border-teal-40 focus:outline-none" :placeholder="t('builderView.appNamePlaceholder')" />
          </div>
          <div>
            <label class="mb-1 block text-sm text-[var(--text-secondary)]">{{ t('builderView.formDescription') }}</label>
            <textarea v-model="appForm.description" rows="3" class="w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-panel)] px-4 py-3 text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:border-teal-40 focus:outline-none" :placeholder="t('builderView.appDescPlaceholder')"></textarea>
          </div>
              ...
              <div class="rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-panel)]/70 p-3">
                <div class="mb-3 flex items-center gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-panel)] px-3 py-2">
                  <span
                    ...
                    :style="{ backgroundColor: appForm.color || '#06B7B2' }"
                  />
                  <div>
                    <p class="text-[11px] uppercase tracking-[0.18em] text-[var(--text-muted)]">{{ t('builderView.currentColor') }}</p>
                    <p class="text-sm font-semibold text-[var(--text-primary)]">{{ appForm.color || '#06B7B2' }}</p>
                  </div>
                </div>

                <div class="grid grid-cols-6 gap-2">
                  <button
                    v-for="color in presetAppColors"
                    :key="color"
                    type="button"
                    class="h-8 w-full rounded-lg border transition"
                    :class="appForm.color === color ? 'border-white/90 ring-2 ring-white/40' : 'border-[var(--border-subtle)] hover:border-white/40'"
                    :style="{ backgroundColor: color }"
                    :title="color"
                    @click="setAppColor(color)"
                  />
                </div>
            ...
            <div>
              ...
              <div class="mb-2 flex items-center gap-3">
                <div
                  ...
                  :style="{ backgroundColor: appForm.color || '#06B7B2' }"
                >
                  <AppIcon :icon="appForm.icon" :size="20" icon-class="text-white" />
                </div>
                <span class="text-sm text-[var(--text-muted)]">{{ appForm.icon || '—' }}</span>
              </div>
              <!-- Grid de íconos seleccionables -->
              <div class="grid grid-cols-6 gap-1.5 max-h-36 overflow-y-auto rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-panel)] p-2">
                <button
                  ...
                  :title="String(name)"
                  @click="appForm.icon = String(name)"
                >
          ...
          <p v-if="appError" class="rounded-xl border border-[var(--priority-urg-bg)]/30 bg-[var(--priority-urg-bg)]/10 px-4 py-3 text-sm text-red-600 dark:text-red-400">{{ appError }}</p>
          <div class="flex gap-3 pt-2">
            <button data-cy="btn-guardar-app" type="submit" class="flex-1 rounded-xl bg-teal px-4 py-3 font-semibold text-[var(--text-primary)] transition hover:bg-teal-40">
              {{ appSaving ? t('builderView.saving') : t('builderView.saveApplication') }}
            </button>
            <button type="button" class="flex-1 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-panel)] px-4 py-3 font-semibold text-[var(--text-primary)] transition hover:bg-[var(--bg-card)]/10" @click="closeAppModal">
              {{ t('common.cancel') }}
            </button>
          </div>
        </form>
...
import {
  ...
  Heart, Bell, Lock, Key, Cpu, Briefcase, Rocket, GitBranch, Terminal
} from 'lucide-vue-next'

const iconComponents: Record<string, object> = {
  Folder, Database, Monitor, Link, Home, Shield, Globe, Code,
  Server, Cloud, Zap, Box, Package, Settings, Users, Star,
